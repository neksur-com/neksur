...
| `NEKSUR_LICENSE_PATH`    | L2/L3 binary REQUIRED | `/etc/neksur/license.json` | Path to signed license manifest (JSON). Required when running `neksur-server-commercial` or `neksur-server-enterprise`. Not required for the L1 BSL Core binary (`neksur-server`). Generated by `cmd/license-gen` (Plan 03-02). |
...
## 3b. Three-binary build matrix (Phase 3 — Plan 03-13)

Neksur ships three binaries per deployment tier (D-3.04 / ADR-002):

| Binary | Build command | Build tags | License required | L2 features | L3 features |
|--------|---------------|------------|-----------------|-------------|-------------|
| `neksur-server` | `make build-core` | (none) | No (BSL Core — free) | No | No |
| `neksur-server-commercial` | `make build-commercial` | `commercial` | Yes (`commercial` or `enterprise` tier) | schemacache, writeconflict, verifier | No |
| `neksur-server-enterprise` | `make build-enterprise` | `commercial enterprise` | Yes (`enterprise` tier) | All L2 | partitionspec, compaction |

```bash
# Build all three in one step:
make build-all
# or individually:
make build-core
...
Artifacts land in `./bin/`. Build requires the private repos to be checked out as siblings:
```
neksur-core/           (this repo)
neksur-commercial/     (sibling; Plan 03-14 bootstrapped)
neksur-enterprise/     (sibling; Plan 03-14 bootstrapped)
```

### License env var

| Variable | Binary | Required | Default | Notes |
|----------|--------|----------|---------|-------|
| `NEKSUR_LICENSE_PATH` | L2 + L3 | **Yes** | `/etc/neksur/license.json` | Path to signed JSON manifest. Generate via `cmd/license-gen`. |
| `NEKSUR_LICENSE_PATH` | L1 (core) | No | n/a | L1 binary ignores this env var entirely. |

The license manifest is signed with ECDSA P-256. Use `make build-core` for local
development without a license; use `make build-commercial` or `make build-enterprise`
with a valid manifest for L2/L3 feature development.

### License rotation walkthrough

To rotate a license file without restarting the server:

1. Generate the new manifest:
   ```bash
   go run ./cmd/license-gen --customer-id <id> --tier commercial \
       --expiry 2027-01-01T00:00:00Z \
       --features write_conflict,schema_cache_broadcaster,continuous_verifier \
       --private-key /etc/neksur/signing-key.pem \
       --out /tmp/new-license.json
   ```

2. Replace the license file atomically (mirrors the fsnotify rename pattern):
   ```bash
   cp /tmp/new-license.json /etc/neksur/license.json.new
   mv /etc/neksur/license.json.new /etc/neksur/license.json
   ```

3. The license fsnotify watcher (Plan 03-13) detects the rename event and
   calls `license.SetManifest` with the new manifest within milliseconds.
   No restart is required.

4. Verify the new manifest is loaded by observing the slog output:
   ```json
   {"level":"info","msg":"license: manifest hot-reloaded","license_id":"...","tier":"commercial"}
   ```

### Tier upgrade walkthrough (L1 → L2 → L3)

To upgrade from the L1 binary to L2 or L3:

1. Obtain a license manifest for the new tier from `cmd/license-gen`.
2. Place the manifest at `NEKSUR_LICENSE_PATH` (default `/etc/neksur/license.json`).
3. Install the new binary (`neksur-server-commercial` or `neksur-server-enterprise`)
   and restart the process.
4. The new binary verifies the license at boot and starts the appropriate L2/L3
   modules (schemacache broadcaster, write-conflict store, verifier, etc.).

Downgrading uses the same process in reverse — replace the binary with the
lower-tier version; the license file is not required for the L1 binary.

### Pitfall 7 — Build-tag drift verification

To verify the L1 binary is clean of private module symbols (Pitfall 7 mitigation):

```bash
make verify-tier-isolation
# Expected: "OK: L1 binary is clean — no private coordination module symbols found."
```

This runs `scripts/ci/binary-symbol-check.sh ./bin/neksur-server`. The same check
runs in CI via `.github/workflows/build-matrix.yml` on every push/PR.
